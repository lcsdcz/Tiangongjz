/**
 * 天宫空间站虚拟漫游 - 主要功能实现
 * 这个文件包含了整个虚拟漫游系统的核心功能实现 陈政我儿子
 */

// DOM元素 - 获取页面上所有需要操作的HTML元素
const launchScreen = document.getElementById('launchScreen'); // 启动屏幕容器
const startBtn = document.getElementById('startBtn'); // "开始探索"按钮
const tourView = document.getElementById('tourView'); // 主要的全景图显示区域
const infoPopup = document.getElementById('infoPopup'); // 信息弹窗容器
const popupTitle = document.getElementById('popupTitle'); // 弹窗标题元素
const popupContent = document.getElementById('popupContent'); // 弹窗内容区域
const closePopup = document.getElementById('closePopup'); // 关闭弹窗的按钮
const prevBtn = document.getElementById('prevBtn'); // 上一个全景图按钮
const nextBtn = document.getElementById('nextBtn'); // 下一个全景图按钮
const panoramaNav = document.getElementById('panoramaNav'); // 全景图导航点容器
const moduleBtns = document.querySelectorAll('.module-btn'); // 所有模块选择按钮
const currentLocation = document.getElementById('currentLocation'); // 当前位置显示
const locationDescription = document.getElementById('locationDescription'); // 位置描述文本

// 漫游状态 - 存储当前漫游的状态信息
let currentModule = 'tianhe'; // 当前选中的模块，默认为天和核心舱
let currentPanoramaIndex = 0; // 当前全景图的索引，默认为第一个全景图
let isRotating = false; // 是否处于自动旋转状态
let rotationInterval; // 自动旋转的定时器引用
